...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("xChr0n0Miner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("xChr0n0Miner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("xChr0n0Miner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("xChr0n0-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running xChr0n0Miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
